...
router = APIRouter(prefix="/api/administrador", tags=["administrador"])
...
async def filtrar(pag: int, lim: int, request: Request):
    ...
    endpoint = f"/administrador/filtrar/{pag}/{lim}" + (f"?{qs}" if qs else "")
...
async def buscar(id: int, request: Request):
    status, content = await call_datos(f"/administrador/{id}", "GET", headers=_auth(request))
    return Response(content=content, status_code=status, media_type="application/json")
...
async def create(request: Request):
    ...
    status, content = await call_datos("/administrador", "POST", body, _auth(request))
    return Response(content=content, status_code=status, media_type="application/json")
...
async def update(identificacion: str, request: Request):
    body = await request.body()
    status, content = await call_datos(f"/administrador/{identificacion}", "PUT", body, _auth(request))
    return Response(content=content, status_code=status, media_type="application/json")


@router.delete("/{identificacion}")
async def delete(identificacion: str, request: Request):
    status, content = await call_datos(f"/administrador/{identificacion}", "DELETE", headers=_auth(request))
    return Response(content=content, status_code=status, media_type="application/json")
