...
# IMPORTANT: register /filtrar/{pag}/{lim} before /{id} to avoid route conflict
...
@router.post("")
def create(body: AdministradorCreate, db: Session = Depends(get_db)):
    status = create_persona(db, RECURSO, ROL, body.model_dump())
    return Response(status_code=status)


@router.put("/{identificacion}")
def update(identificacion: str, body: AdministradorUpdate, db: Session = Depends(get_db)):
